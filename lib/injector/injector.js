...
injector = {
  ...
  loadNested: function(services, config) {
    var defn, existing, name, nextService, rebuild, _i, _len, _results;
    services.push({});
    _results = [];
    for (name in config) {
      defn = config[name];
      ...
      if (defn.length > 1) {
        _results.push(services[services.length - 1][defn[0]] = nextService[name]);
      } else {
        _results.push(services[services.length - 1][name] = nextService);
      }
    }
    return _results;
  }
