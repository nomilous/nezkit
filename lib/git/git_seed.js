...
var GitRepo, GitSeed, colors, fs, series;
...
GitSeed = (function() {
  ...
  GitSeed.init = function(root, PackagePlugin) {
    ...
    return find.on('end', function() {
      var packages, path, seq, tree, _i, _len;
      packages = [];
      seq = 0;
  ...
  function GitSeed(root, PackagePlugin, list) {
    this.root = root;
    this.control = "" + this.root + "/.git-seed";
    if (list instanceof Array) {
      this.array = list;
    } else if (typeof list === 'undefined') {
      this.array = this.load(PackagePlugin);
    }
  }
  ...
  GitSeed.prototype.load = function(PackagePlugin) {
